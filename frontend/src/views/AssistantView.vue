  ...
  <div class="adm-fade-in assistant-view">
            ...
            >
              ...
              <td>
                <div class="account-cell">
                  <span v-if="a.userId" class="status-tag linked" :title="a.userId">
                    <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"/></svg>
                    {{ getAccountDisplayName(a.userId) }}
                  </span>
                  <span v-else class="status-tag unlinked">未关联</span>
                  <button class="adm-btn adm-btn-ghost adm-btn-xs" @click="openAccountDialog(a)">
                    {{ a.userId ? '更换' : '关联' }}
                  </button>
                </div>
              </td>
    ...
    <div v-if="showAccountDialog" class="modal-mask" @click.self="closeAccountDialog">
      <div class="modal-box" style="width: 480px;">
        ...
        <div class="modal-body">
          <div v-if="accountDialogAssistant" class="ast-info-banner">
            <div class="ast-avatar" :style="{ background: getAvatarGradient(getDisplayName(accountDialogAssistant)) }">{{ getAvatarChar(getDisplayName(accountDialogAssistant)) }}</div>
            <div>
              <div class="info-name">{{ getDisplayName(accountDialogAssistant) }}</div>
              <div class="info-sub">{{ getPhone(accountDialogAssistant) }} · 当前 {{ accountDialogAssistant.subjects.length }} 个科目</div>
              <div v-if="accountDialogAssistant.userId" class="info-sub account-id">当前关联: {{ accountDialogAssistant.userId }}</div>
            </div>
          </div>
          <div class="form-group">
        ...
        <div class="modal-footer">
          ...
          <button class="adm-btn adm-btn-primary" :disabled="!accountDialogForm.selectedUserId || accountDialogSaving" @click="saveAssistantAccount">
...
const accountDialogAssistant = ref<AssistantAccountDto | null>(null)
...
function openAccountDialog(a: AssistantAccountDto) {
  accountDialogAssistant.value = a
  accountDialogMode.value = a.userId ? 'change' : 'link'
...
function closeAccountDialog() {
  ...
  accountDialogAssistant.value = null
  accountDialogForm.value = { keyword: '', selectedUserId: '' }
  accountDialogUsers.value = []
}
...
async function searchAccountDialogUsers() {
  ...
  try {
    ...
    const currentUserId = accountDialogAssistant.value?.userId
...
async function saveAssistantAccount() {
  if (!accountDialogAssistant.value?.userId && accountDialogMode.value === 'link') {
    ElMessage.warning('该助教暂无关联账户，请先撤销后重新授予')
    return
  }
  ...
  const assistant = accountDialogAssistant.value!
  accountDialogSaving.value = true
  try {
    await assistantPortalClient.updateAssistantUserId(assistant.userId!, accountDialogForm.value.selectedUserId)
