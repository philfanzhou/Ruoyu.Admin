...
| HR-03 | OssUploadRecordController、MistakeController N+1 查询 | 多个 Controller            | 待修复 | <br />   |
| HR-04 | ImageController 路径遍历                               | ImageController.cs       | 待修复 | <br />   |
| HR-05 | OssAuditController fire-and-forget Task.Run        | OssAuditController.cs    | 待修复 | <br />   |
...
| ID    | 问题                                                                                    | 位置                        | 状态  | 批复     |
| ----- | ------------------------------------------------------------------------------------- | ------------------------- | --- | ------ |
| HR-13 | IOssService 凭证权限降级方案：生产环境如何配置只读+有限写的 OSS 凭证？SeaweedFS 是否支持细粒度权限？                      | IOssService / appsettings | 待审查 | <br /> |
| HR-14 | Student 通用 gRPC 接口是否支持管理端身份调用（无需 studentId）？若不支持需补充通用接口                               | Student Service           | 待审查 | <br /> |
...
| HR-17 | MistakeGrpcService.GetAllReferencedImagePaths 移除后，Mistake 通用 gRPC 分页接口是否支持按需返回图片路径字段？ | Mistake Service           | 待审查 | <br /> |
