...
为 admin_portal 项目优化 docs/ 目录文档体系，基于新增的文档颗粒度原则：
- 顶层文档保持薄（总览 + 导航 + 边界）
- 新增内容优先下沉到最小责任单元
- 01-FEATURE.md 严格遵循 1 条核心用户故事模板
- 02-SPEC.md 严格遵循 SPEC 模板（非架构文档）
- 补充 development/ 目录提供跨模块开发执行支持
...
| OssAudit/02-SPEC.md | 重写 | 从架构文档重写为 SPEC 模板（FR/AC/NFR/测试策略） |
| development/ | 新增 4 文件 | README + LocalSetup + RunAndDebug + Verification |
| docs/README.md | 更新 | 新增 development/ 导航入口 |
...
- **为什么仍无法完全确认**: proto 文件位于项目外部，虽然已读取但未在文档中完整引用每个 message 的字段定义
...
- **建议人工复核**: 确认并发控制策略是否符合预期，特别是在 SQLite 测试环境下
...
- **编号**: ARN-04
- **问题描述**: `GetIdentityAccountsBatch` 在 Identity Service 不可用时静默返回空列表，可能导致前端显示不完整数据。
...
- **为什么仍无法完全确认**: 代码中有 pathMapping 缓存和逐张处理逻辑，但缺少整体回滚机制
- **建议人工复核**: 确认是否需要事务性保证或补偿机制

### 3.7 deployment.md 中重复了数据库表结构

- **编号**: ARN-07
- **问题描述**: `deployment.md` 第 290-307 行包含了 `oss_audit_records` 表的 DDL 和索引定义，与 `database/tables/oss_audit_records.md` 形成重复事实源。
...
| SubjectConstants.IsValid 和 DateFormat | StudentManagement/OpenSubjectManagement/03-DESIGN.md | [待确认] - 常量定义在 Ruoyu.Study.Common 中 |
| IOssService.CopyObjectAsync/DeleteAsync 行为 | MistakeManagement/03-DESIGN.md | [推断] - 基于 MistakeController 调用推断 |
| OssAuditWorker 在 SQLite 环境下的行为 | OssAudit/OssAudit/02-SPEC.md | [推断] - SQLite 不注册为 HostedService |
| Identity Service HTTP API 路径 | Integration/IdentityService/IdentityProxy/03-DESIGN.md | [推断] - 基于中间件路径重写推断 |
| Teacher Portal HTTP API 路径 | Integration/TeacherPortal/TeacherPortalProxy/03-DESIGN.md | [推断] - 基于中间件路径重写推断 |
| Resolved 记录跳过引用校验 | OssAudit/OssAudit/02-SPEC.md | [推断] - 基于代码逻辑推断 |
...
当前测试仅覆盖：
- Model 构造函数测试（ModelTests.cs）
- OssUploadRecordController 的 LegacyCheck/LegacyClean 测试

缺失的测试详见 [development/Verification.md](./development/Verification.md) 的测试覆盖现状表。

### 5.2 deployment.md 与 database/ 重复事实源

`deployment.md` 中的表结构 DDL 与 `database/tables/` 下的文档存在重复。建议后续将 `deployment.md` 中的 DDL 替换为指向 `database/` 的链接，保持唯一事实源。

### 5.3 文档与代码同步维护

当前文档基于代码快照生成，后续代码变更可能导致文档过时。建议：
- 在 CI 中添加文档链接检查
- 在 PR 模板中添加"是否需要更新文档"检查项
- 对 04-TASKS.md 中的 todo 项进行跟踪

### 5.4 前端文档

前端文档 `frontend/docs/frontend-spec.md` 已存在且内容完整，本次未重复创建。建议后续考虑将其纳入 docs/ 体系统一管理。
